...
groll-cli脚手架工具就是为我们搭建了开发所需要的环境，开发者只需要在生成的项目结构的基础上进行开发即可，非常简单。为了高效的使用 groll-cli, 首先熟悉使用 groll-cli 脚手架生成的项目结构。
...
- `html.js` 用于对 html 文件进行构建，收集依赖的静态资源，并对静态资源（图片、样式和脚本）的 URL 地址，并根据构建环境，对 HTML 文件进行压缩等处理。输出 HTML 文件到 dist/1.x.x/ 构建目录
- `img.js` 图片资源进行复制、添加 MD5 后缀、压缩等处理。输出图片资源到 dist/version/img 构建目录。
- `css.js` 使用 postcss 对指定的 css 文件进行编译、添加 MD5 后缀、 资源内嵌（将指定的 css 文件以 style 标签形式内嵌，提升首屏展示效果）等处理。根据构建环境，对 css 资源进行压缩等处理，输出 css 文件到 dist/version/css 构建目录。
- `rollup.js` 使用 rollup 模块打包器对 js 进行打包，将符合 ES 规范的小块代码编译成大块复杂的代码。支持加载 Commonjs 规范的第三方公用库。根据构建环境，对打包过后的 js 进行压缩等处理，输出相应文件到 dist/version/js 构建目录。
- `server.js` 使用 Browsersync 构建 web 服务，同时设置相关开发环境的服务代理。Browsersync 能让浏览器实时、快速响应您的文件更改（html、js、css、sass、less等）并自动刷新页面。更重要的是 Browsersync 可以同时在PC、平板、手机等设备下进项调试。
- `utils.js` 提供基础的操作工具函数，例如：情况构建文件夹，计算路径配置等。
...
- `proxyTable`: 代理现有的服务相关配置;
	...
	- `changeOrigin`：默认 true ，是否需要改变原始主机头为目标 URL；
	- `pathRewrite`：重写请求，比如: api/old-path，那么请求会被解析为 /api/new-path 。
- `zip`： 是否启用 zip 对静态资源压缩，并生成相应的 zip 文件，默认 false；
- `ftp`：相关配置
	- `enabled`: 是否启用，默认 false；
	- `host`: 服务主机名；
	- `user`: 登陆主机的用户名（必须有读写权限）；
	- `password`：登陆主机用户密码；
	- `parallel`：并行传输数，默认为 3
...
- `version` 当前构建版本，构建目录会根据构建版本号创建不同的文件，如果不期望使用版本号，可以置空。
- `entry` 构建页面入口（构建以页面为单位），必选项。可以以列表的方式列出每次构建的文件，也可以通过 glob 规则指定文件。例如：
